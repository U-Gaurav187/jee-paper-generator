...
export default function WizardStep6_JEE2ColumnPrintPDF({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Action Bar (Web Screen Only - Hidden during print) */}
      ...
      {/* Pure A4 Paper Document Node Target for PDF Generation */}
      <div className="print-only-container flex justify-center overflow-x-auto py-4 bg-slate-950 sm:p-6 rounded-2xl">
        
        <div
          ref={paperRef}
          className="jee-paper-document relative bg-white text-slate-950 p-6 sm:p-10 rounded shadow-2xl max-w-[210mm] w-full text-[10.5pt] leading-normal font-sans"
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <div className="border-b-2 border-slate-900 pb-3 mb-4 text-center">
              ...
              <div className="flex items-center justify-between text-xs font-semibold text-slate-800 mt-3 pt-2 border-t border-slate-300">
            ...
            <div className="bg-slate-50 border border-slate-300 rounded p-2.5 mb-5 text-[9pt] text-slate-800 leading-snug">
              <span className="font-bold uppercase text-[9.5pt] block mb-1">Read the Following Instructions Carefully:</span>
            ...
            <div className="jee-paper-preview">
              {groupedQuestions.map((sec) => (
                <div key={sec.type} className="mb-5">
                  ...
                  <div className="span-all-columns bg-slate-100 border-y-2 border-slate-800 py-1 px-2 mb-3 font-bold text-[9.5pt] uppercase tracking-wide text-slate-900 text-center">
                    {sec.title}
                  </div>
                  ...
                  {sec.questions.map((q) => {
                    ...
                    return (
                      <div key={q.id} className="jee-question-card text-[10pt]">
                        
                        {/* Paragraph Passage Stem */}
                        {q.type === 'paragraph' ? (
                          <div className="mb-3">
                            <div className="bg-slate-50 border-l-2 border-slate-800 p-2 text-[9pt] mb-2 text-slate-900 italic font-serif leading-snug">
                              <KaTeXRenderer text={q.paragraphText} />
                            </div>

                            {q.subQuestions?.map((subQ, subIdx) => (
                              <div key={subQ.subId} className="mt-2 text-[9.5pt]">
                                ...
                                {subQ.options && (
                                  <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 mt-1 text-[9pt]">
                          ...
                          <div>
                            ...
                            {q.chemStructure && (
                              <div className="my-2 text-center">
                                <SmilesRenderer smiles={q.chemStructure} width={140} height={100} />
                              </div>
                            )}
                            ...
                            {/* SCQ / MCQ Options */}
                            {q.options && (
                              <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 mt-1.5 text-[9.5pt] text-slate-900">
                            ...
                            {q.type === 'matrix_match' && q.matrixMatch && (
                              <div className="mt-2 text-[9pt] border border-slate-300 rounded p-1.5 bg-slate-50/60">
                                <div className="grid grid-cols-2 gap-1.5">
                                  <div>
                                    <span className="font-bold text-slate-800 uppercase text-[8.5pt] border-b border-slate-300 block mb-1">Column I</span>
                                    {q.matrixMatch.column1.map((item, i) => (
                                      <div key={i} className="py-0.5 text-slate-900 leading-tight"><KaTeXRenderer text={item} /></div>
                                    ))}
                                  </div>
                                  <div>
                                    <span className="font-bold text-slate-800 uppercase text-[8.5pt] border-b border-slate-300 block mb-1">Column II</span>
            ...
            {paperMode === 'question_with_solutions' && (
              <div className="mt-10 pt-6 border-t-2 border-slate-900 page-break-before">
                ...
                <div className="text-center mb-5">
                  ...
                  <p className="text-[9pt] text-slate-600 font-mono">{metadata.testTitle}</p>
                </div>

                {/* Grid Table of Answer Keys */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 font-mono text-[8.5pt] mb-8">
                ...
                <div className="space-y-3 text-[9.5pt]">
                  {paperQuestions.map((q, idx) => (
                    <div key={q.id} className="border-b border-slate-200 pb-2.5">
                      <span className="font-bold font-mono text-indigo-900">
                        Question {idx + 1} ({q.subject} - {q.chapter}):
                      </span>
                      <div className="mt-1 text-slate-800 leading-relaxed font-sans bg-slate-50 p-2 rounded border border-slate-200 text-[9pt]">
            ...
            <div className="mt-8 pt-3 border-t border-slate-300 flex items-center justify-between text-[8pt] text-slate-500 font-mono">
